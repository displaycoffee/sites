	...
	*/
	...
	function insprvw_recent_reviews( $atts ) {	
	    ...
	    $a = shortcode_atts( array(
	        'types' => array( 'insprvw-book-review', 'insprvw-movie-review', 'insprvw-tv-review' ),
	        'amount'    => '5'
	    ), $atts );	
		...
		if ( $insprvw_query->have_posts() ) {	
			...
			while ( $insprvw_query->have_posts() ) {
				...
				if ( has_post_thumbnail() ) {				
					...
					$review_thumbnail .= '<img src="' . plugins_url( 'inspire-reviews/assets/images/default-square-thumbnail.png', '' ) . '" width="400" height="400" alt="Image Coming Soon" />';
				}
				...
				$review .= $review_date;
				$review .= $review_thumbnail;
				$review .= $review_title;
				$review .= '<hr />';
				$review .= '<div class="review-excerpt">' . insprvw_excerpt() . '</div>';
				$review .= '</div>';
			}
			// Reset post data
			wp_reset_postdata();
			...
			// Display review wrapper block - END
