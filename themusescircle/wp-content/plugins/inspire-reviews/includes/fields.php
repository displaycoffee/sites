<?php  
    ...
    function insprvw_display_fields( $field, $value ) {
        switch ( $field['type'] ) {
            // Text and url
            case 'text':
            case 'url':
            ...
            case 'color':
                $color = $value ? $field['validate']( $value ) : __( 'No color selected.', 'inspire-reviews' );
                echo '<input type="text" name="' . $field['name'] . '" id="' . $field['id'] . '" value="' . $field['validate']( $value ) . '" class="color-select" />';
                echo '<div class="current-color"><strong>' . __( 'Current Color:', 'inspire-reviews' ) . '</strong> ' . $color . '</div>';
                break;

            // Wordpress Media library    
            case 'media':
                $image = $value ? '<div class="image-preview"><strong>' . __( 'Current Image:', 'inspire-reviews' ) . '</strong><img src="' . $field['validate']( $value ) . '" /></div>' : '';
                echo '<div class="media-field">';
                echo $image;        
                echo '<input type="url" name="' . $field['name'] . '" id="' . $field['id'] . '" value="' . $field['validate']( $value ) . '" /><br />';
                echo '<input type="button" class="image-select button" value="' . __( 'Choose or Upload an Image', 'inspire-reviews' ) . '" />';
                echo '<input type="button" class="image-reset button" value="' . __( 'Clear Image', 'inspire-reviews' ) . '" />';
                echo '</div>';
                break; 
    ...
    function insprvw_display_multitext( $field, $option, $value ) {
        echo '<div class="text">';
        echo '<label for=' . $option['id'] . '>';
        echo $option['label'];
        echo '</label>';
    ...
    function insprvw_display_multicheck( $field, $option, $checked ) {
    ...
    function insprvw_display_description( $field ) {
