	...
	*/
	public function validate_profile_field(&$field_value, $field_data)
	{
		...
		if ($field_data['field_required'] && $field_data['field_minlen'] && $field_size < $field_data['field_minlen'])
		{
			...
			$option_count = (int) $field_data['field_minlen'];
			$plural = $option_count > 1 ? 's' : '';
			return 'You have not selected enough options for the field “' . $field_data['lang_name'] . '”. At least ' . $option_count . ' option' . $plural . ' must be selected.';
		}
		...
		if ($field_data['field_maxlen'] && $field_size > $field_data['field_maxlen'])
		{
			...
			$option_count = (int) $field_data['field_maxlen'];
			$plural = $option_count > 1 ? 's' : '';
			return 'You have selected too many options on field “' . $field_data['lang_name'] . '”. At most ' . $option_count . ' option' . $plural . ' may be selected.';
		}
